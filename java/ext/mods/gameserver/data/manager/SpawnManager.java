...
public class SpawnManager implements IXmlReader
{
    ...
    private final Map<Integer, List<ASpawn>> _spawnsByNpcId = new ConcurrentHashMap<>();
    private final Map<String, List<ASpawn>> _spawnsByAlias = new ConcurrentHashMap<>();
    ...
    public void load()
    {
        ...
        LOGGER.info("Indexed {} unique NPC IDs for instant lookup.", _spawnsByNpcId.size());
    }
        ...
        forEach(doc, "list", listNode ->
        {
            ...
            forEach(listNode, "npcmaker", npcmakerNode ->
            {
                ...
                forEach(npcmakerNode, "npc", npcNode ->
                {
                    ...
                    try
                    {
                        MultiSpawn multiSpawn = new MultiSpawn(maker, template, total, respawnDelay, respawnRandom, privateData, spawnMemo, coords2, spawnData);
                        spawns.add(multiSpawn);
                        
                        _spawnsByNpcId.computeIfAbsent(npcId, k -> Collections.synchronizedList(new ArrayList<>())).add(multiSpawn);
                        _spawnsByAlias.computeIfAbsent(template.getAlias().toLowerCase(), k -> Collections.synchronizedList(new ArrayList<>())).add(multiSpawn);
                    }
                    catch (Exception e)
                    {
    ...
    public void reload()
    {
        ...
        _makers.clear();
        _spawns.clear();
        _spawnsByNpcId.clear();
        _spawnsByAlias.clear();
    ...
    public void addSpawn(Spawn spawn)
    {
        ...
        _spawnsByNpcId.computeIfAbsent(spawn.getNpcId(), k -> Collections.synchronizedList(new ArrayList<>())).add(spawn);
        if (spawn.getTemplate() != null && spawn.getTemplate().getAlias() != null) {
            _spawnsByAlias.computeIfAbsent(spawn.getTemplate().getAlias().toLowerCase(), k -> Collections.synchronizedList(new ArrayList<>())).add(spawn);
        }
    }
                    ...
                    BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile)))
                {
                    ...
                    while ((currentLine = reader.readLine()) != null)
                    {
                        if (currentLine.contains("</list>"))
                        {
                            ...
                            writer.write("      <npc id=\"" + spawnId + "\" pos=\"" + spawnLoc + "\" total=\"" + 1 + "\" respawn=\"" + respawn + "sec" + respawnRndString + "\" /> \n");
    ...
    public void deleteSpawn(Spawn spawn)
    {
        ...
        List<ASpawn> list = _spawnsByNpcId.get(spawn.getNpcId());
        if (list != null) list.remove(spawn);
        
        if (spawn.getTemplate() != null && spawn.getTemplate().getAlias() != null) {
            List<ASpawn> aliasList = _spawnsByAlias.get(spawn.getTemplate().getAlias().toLowerCase());
            if (aliasList != null) aliasList.remove(spawn);
        }
    }
    ...
    public final ASpawn getSpawn(int npcId)
    {
        ...
        List<ASpawn> spawns = _spawnsByNpcId.get(npcId);
        ASpawn result = (spawns != null && !spawns.isEmpty()) ? spawns.get(0) : null;
    ...
    {
        List<ASpawn> spawns = _spawnsByAlias.get(npcAlias.toLowerCase());
        return (spawns != null && !spawns.isEmpty()) ? spawns.get(0) : null;
    }
